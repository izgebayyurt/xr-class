...
A. The idea
1. You put on the headset and play a game that tests how good you are at judging distances in metres.
2. After two minutes a visitor should feel how bad (or good!) their sense of "one metre" actually is — and start getting better at it.
3. Tier: 3 (Rules). I know this is the risky one but my interactions are just point-and-click.

B. The space
4. Posture: standing in place (no teleport — the distances only mean something if you stay put).
5. First thing you see: a glowing marker post standing on the floor somewhere in front of you (the game moves it every round — sometimes really close, sometimes pretty far). Above it nothing. Across the room at eye level, a sign with the title and the score.
6. Everything else: four answer pads floating in a row in front of me, a bit below chest height at arm's reach. Each shows a distance like "2.5 m". Wait — they can't always show the same four numbers, so: each round the pads show four possible distances and exactly one is correct.
7. Nothing behind me.

C. Interactions (max 3)
1. You do: point + trigger — to: an answer pad — and it: locks my guess. The pad I picked lights green if right / red if wrong (and the correct one lights green), a tape measure appears on the floor from my feet to the post with marks at every metre so I can SEE the real distance, and a happy or sad tone plays — then: after a few seconds the tape disappears and the next round starts.
2. You do: point + trigger — to: the sign, after the game ends — and it: starts a new game — then: repeatable.
3. (blank)

D. Rules and state
8. Tracked: round number and score, on the sign across the room, like "Round 2 of 5 — Score 1".
9. Ends after 5 rounds. The sign shows my final score out of 5 and something encouraging, and says to click the sign to play again.
10. Start over: click the sign.

E. Look, feel, words
11. Mood: playful, arcade-y, bright. Colours: teal, orange, white.
12. Text that must appear exactly: "How Far Is That?"

F. Scope guard
13. Out of scope: no timer pressure, no leaderboard, no fancy sounds, no teleporting.
14. Cut first: the tape measure (just show the true distance as text instead).
15. Don't change: the distances must actually be random each round, and the correct answer can't always sit on the same pad.
